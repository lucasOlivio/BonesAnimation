...
namespace MyEngine
{
    ...
    void VAOManager::m_LoadVAOData(sMesh* pMesh, bool hasNormals, bool hasTexture, bool bIsDynamicBuffer)
    {
        ...
        GLint vBoneIds_location = pShader->GetAL("vBoneIds");
        GLint vBoneWeights_location = pShader->GetAL("vBoneWeights");
        ...
        glVertexAttribPointer(vBoneIds_location, 4,
                                GL_FLOAT, GL_FALSE,
                                sizeof(sVertex),
                                (void*)offsetof(sVertex, b1));
        
        glEnableVertexAttribArray(vBoneWeights_location);
        glVertexAttribPointer(vBoneWeights_location, 4,
                                GL_FLOAT, GL_FALSE,
                                sizeof(sVertex),
                                (void*)offsetof(sVertex, w1));
